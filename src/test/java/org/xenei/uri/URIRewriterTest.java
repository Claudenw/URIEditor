...
public class URIRewriterTest {
	...
	public void testRewriteSkip() throws URISyntaxException
	{
		...
		.setFragment( FRAGMENT_PATTERN );
		rewriter = new URIRewriter( patternReplacer );
		URI uri = rewriter.rewrite( new URI("http://example.com:80/foo/foo2#bar"));
		assertEquals( "http", uri.getScheme());
	...
	public void testRewriteBadPattern() throws URISyntaxException
	{
		...
		try {
			URI uri = rewriter.rewrite( testURI );
			fail( "Should have thrown URISyntaxException");
		}
		catch (URISyntaxException e)
		{
			assertEquals( "Subpattern 3 does not exist in ^(.).+(.)$ matching http: http://bax.example.com:80/foo/foo2#bar", e.getMessage() );
		}
	}
}
